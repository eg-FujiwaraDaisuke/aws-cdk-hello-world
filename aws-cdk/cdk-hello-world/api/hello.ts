...
export class HelloApi extends Construct {
    public constructor(scope: Construct, context: Context) {
        ...
        const apiResource = api.root.getResource(apiPath) ?? api.root.addResource(apiPath);
        const stackResource =
            apiResource.getResource(`${stackType}`) ?? apiResource.addResource(`${stackType}`);
        const resource = stackResource.getResource(apiName) ?? stackResource.addResource(apiName);
